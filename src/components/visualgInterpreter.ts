...
export const TokenType = {
	...
	REPITA: "REPITA",
	ATECOMANDO: "ATECOMANDO", // “REPITA ... ATE”
	ESCOLHA: "ESCOLHA",
	CASO: "CASO",
	OUTROCASO: "OUTROCASO",
	...
	PAUSA: "PAUSA",
	LIMITELA: "LIMITELA", // “pausa” e “limpatela”
	ESCREVA: "ESCREVA",
	ESCREVAL: "ESCREVAL",
	LEIA: "LEIA",
		...
		switch (c) {
			...
			default:
				if (this.isDigit(c)) {
					...
					throw new Error(
						`[Linha ${this.line}] Caractere inesperado: '${c}'`
					);
				}
				break;
		}
...
class Parser {
	...
	private varDeclaration(): VarDecl {
		...
		do {
			const token = this.consume(
				...
				"Esperava identificador em declaração de variável."
			);
	...
	private constDeclaration(): ConstDecl {
		...
		const nameToken = this.consume(
			TokenType.IDENTIFIER,
			"Esperava identificador em declaração de constante."
		);
		...
		this.consume(
			TokenType.LESS,
			"Esperava '<' em declaração de constante."
		);
		this.consume(
			TokenType.MINUS,
			"Esperava '-' em declaração de constante."
		); // sob hipótese de lexing, '<-' vira LESS e MINUS
		const valueExpr = this.expression();
		this.consume(
			TokenType.SEMICOLON,
			"Esperava ';' após declaração de constante."
		);
		return new ConstDecl(
			nameToken.lexeme,
			typeToken.lexeme.toLowerCase(),
			valueExpr
		);
	}
	...
	private procDeclaration(): ProcDecl {
		...
		const nameToken = this.consume(
			TokenType.IDENTIFIER,
			"Esperava nome do procedimento."
		);
		this.consume(
			TokenType.LEFT_PAREN,
			"Esperava '(' após nome do procedimento."
		);
		const params = this.parseParams();
		this.consume(
			TokenType.RIGHT_PAREN,
			"Esperava ')' após parâmetros do procedimento."
		);
		this.consume(
			TokenType.SEMICOLON,
			"Esperava ';' após PROCEDIMENTO(...)."
		);
		...
		if (!this.check(TokenType.RIGHT_PAREN)) {
			do {
				const paramName = this.consume(
					TokenType.IDENTIFIER,
					"Esperava nome de parâmetro."
				).lexeme;
				this.consume(
					TokenType.COLON,
					"Esperava ':' após nome de parâmetro."
				);
				const typeToken = this.consumeTypeName();
				params.push({
					name: paramName,
					typeName: typeToken.lexeme.toLowerCase(),
				});
			} while (this.match(TokenType.COMMA));
		}
		return params;
	...
	private consumeTypeName(): Token {
		...
		throw new Error(
			`[Linha ${
				this.peek().line
			}] Esperava tipo de dado (inteiro, real, lógico, caractere, literal).`
		);
	}
	...
	private statement(): Stmt {
		...
		if (this.match(TokenType.IDENTIFIER)) {
			...
			if (this.match(TokenType.LESS)) {
				...
				this.consume(
					TokenType.MINUS,
					"Esperava '-' após '<' em atribuição."
				);
				const expr = this.expression();
				this.consume(
					TokenType.SEMICOLON,
					"Esperava ';' após atribuição."
				);
		...
		if (this.match(TokenType.ESCREVA, TokenType.ESCREVAL)) {
			...
			this.consume(
				TokenType.LEFT_PAREN,
				"Esperava '(' após ESCREVA/ESCREVAL."
			);
			const args = this.parseArgs(); // chama parseArgs
			this.consume(
				TokenType.RIGHT_PAREN,
				"Esperava ')' após lista de argumentos de ESCREVA."
			);
		...
		if (this.match(TokenType.LEIA)) {
			...
			const nameToken = this.consume(
				TokenType.IDENTIFIER,
				"Esperava nome de variável em LEIA."
			);
		...
		throw new Error(
			`[Linha ${this.peek().line}] Comando/declaração não reconhecido.`
		);
	}
	...
	private ifStatement(): IfStmt {
		...
		this.consume(
			TokenType.FIMSE,
			"Esperava 'FIMSE' após bloco de SE/SENAO."
		);
	...
	private switchCaseStatement(): SwitchCaseStmt {
		...
		const varToken = this.consume(
			TokenType.IDENTIFIER,
			"Esperava variável após ESCOLHA."
		);
		...
		this.consume(
			...
			"Esperava 'FIMESCOLHA' para fechar ESCOLHA."
		);
	...
	private whileStatement(): WhileStmt {
		...
		this.consume(
			TokenType.FIMENQUANTO,
			"Esperava 'FIMENQUANTO' após loop."
		);
		if (this.match(TokenType.SEMICOLON)) {
			/* ignora */
		}
	...
	private primary(): Expr {
		...
		if (this.match(TokenType.IDENTIFIER)) {
			...
			if (this.match(TokenType.LEFT_PAREN)) {
				...
				this.consume(
					TokenType.RIGHT_PAREN,
					"Esperava ')' após argumentos de função."
				);
		...
		if (this.match(TokenType.INT, TokenType.REAL, TokenType.CARAC)) {
			...
			this.consume(
				TokenType.LEFT_PAREN,
				`Esperava '(' após ${typeName}.`
			);
			const expr = this.expression();
			this.consume(
				TokenType.RIGHT_PAREN,
				`Esperava ')' após expressão em ${typeName}.`
			);
	...
	private literalExpression(): Expr {
		...
		throw new Error(
			`[Linha ${this.peek().line}] Esperava literal para CASO.`
		);
	}
	...
	private consume(type: TokenType, message: string): Token {
		...
		throw new Error(
			`[Linha ${this.peek().line}] ${message} (achado: ${
				this.peek().type
			}).`
		);
	}
...
class Environment {
	...
	public castValue(typeName: string, rawValue: any): ValueType {
		...
		switch (typeName) {
			case "inteiro":
				...
				if (typeof rawValue === "string") {
					...
					if (isNaN(parsed))
						throw new Error(
							`Não foi possível converter '${rawValue}' para inteiro.`
						);
					return parsed;
				}
			...
			case "real":
				...
				if (typeof rawValue === "string") {
					...
					if (isNaN(parsed))
						throw new Error(
							`Não foi possível converter '${rawValue}' para real.`
						);
					return parsed;
				}
			...
			case "logico":
				...
				if (typeof rawValue === "string") {
					...
					if (lowered === "verdadeiro" || lowered === "true")
						return true;
					if (lowered === "falso" || lowered === "false")
						return false;
					throw new Error(
						`Não foi possível converter '${rawValue}' para lógico.`
					);
				}
				break;
			case "caractere":
		...
		throw new Error(
			`Tipo incompatível: não é possível converter valor '${rawValue}' para tipo ${typeName}.`
		);
	}
}
...
const builtInFunctions: Record<string, (args: ValueType[]) => ValueType> = {
	...
	abs: ([x]) => {
		if (typeof x !== "number")
			throw new Error("Abs(): argumento não é número.");
		return Math.abs(x);
	},
	// Log(x): log natural
	log: ([x]) => {
		if (typeof x !== "number")
			throw new Error("Log(): argumento não é número.");
		return Math.log(x);
	},
	// Sen(x): seno (em radianos)
	sen: ([x]) => {
		if (typeof x !== "number")
			throw new Error("Sen(): argumento não é número.");
		return Math.sin(x);
	},
	// Cos(x)
	cos: ([x]) => {
		if (typeof x !== "number")
			throw new Error("Cos(): argumento não é número.");
		return Math.cos(x);
	},
	// Tan(x)
	tan: ([x]) => {
		if (typeof x !== "number")
			throw new Error("Tan(): argumento não é número.");
		return Math.tan(x);
	},
	// Exp(x): e^x
	exp: ([x]) => {
		if (typeof x !== "number")
			throw new Error("Exp(): argumento não é número.");
		return Math.exp(x);
	},
	// Pot(base, expoente)
	pot: ([base, expv]) => {
		if (typeof base !== "number" || typeof expv !== "number")
			throw new Error("Pot(): argumentos não são numéricos.");
		return Math.pow(base, expv);
	},
	// Quad(x): x^2
	quad: ([x]) => {
		if (typeof x !== "number")
			throw new Error("Quad(): argumento não é número.");
		return x * x;
	},
	// Cotan(x) = 1/tan(x)
	cotan: ([x]) => {
		if (typeof x !== "number")
			throw new Error("Cotan(): argumento não é número.");
		return 1 / Math.tan(x);
	},
	// ArcSen(x) = arcsin(x)
	arcsen: ([x]) => {
		if (typeof x !== "number")
			throw new Error("ArcSen(): argumento não é número.");
		return Math.asin(x);
	},
	// ArcCos(x)
	arccos: ([x]) => {
		if (typeof x !== "number")
			throw new Error("ArcCos(): argumento não é número.");
		return Math.acos(x);
	},
	// RaizQ(x) = sqrt(x)
	raizq: ([x]) => {
		if (typeof x !== "number")
			throw new Error("RaizQ(): argumento não é número.");
		if (x < 0)
			throw new Error(
				"RaizQ(): não pode tirar raiz quadrada de número negativo."
			);
		return Math.sqrt(x);
	},
	...
	div: ([x, y]) => {
		if (typeof x !== "number" || typeof y !== "number")
			throw new Error("Div(): argumentos não são números.");
	...
	mod: ([x, y]) => {
		if (typeof x !== "number" || typeof y !== "number")
			throw new Error("Mod(): argumentos não são números.");
	...
	asc: ([c]) => {
		if (typeof c !== "string" || c.length === 0)
			throw new Error("Asc(): argumento inválido.");
		return c.charCodeAt(0);
	},
	// Carac(num): retorna caractere de código num
	carac: ([num]) => {
		if (typeof num !== "number")
			throw new Error("Carac(): argumento não é número.");
	...
	int: ([x]) => {
		...
		if (typeof x === "string") {
			...
			if (isNaN(parsed))
				throw new Error("Int(): string não conversível.");
			return parsed;
		}
	...
	compr: ([str]) => {
		if (typeof str !== "string")
			throw new Error("Compr(): argumento não é string.");
		return str.length;
	},
	// Maiusc(str): converte para maiúsculas
	maiusc: ([str]) => {
		if (typeof str !== "string")
			throw new Error("Maiusc(): argumento não é string.");
		return str.toUpperCase();
	},
	// Minusc(str): converte para minúsculas
	minusc: ([str]) => {
		if (typeof str !== "string")
			throw new Error("Minusc(): argumento não é string.");
		return str.toLowerCase();
	},
	// NumPCarac(str, posicao): retorna código numérico do caractere na posicao (1-based)
	numpcarac: ([str, posicao]) => {
		if (typeof str !== "string" || typeof posicao !== "number")
			throw new Error("NumPCarac(): argumentos inválidos.");
		const i = Math.trunc(posicao) - 1;
		if (i < 0 || i >= str.length)
			throw new Error("NumPCarac(): posição fora de alcance.");
		return str.charCodeAt(i);
	},
	// CaracPNum(num): converte num para caractere (mesma de carac)
	caracpnum: ([num]) => {
		if (typeof num !== "number")
			throw new Error("CaracPNum(): argumento não é número.");
		return String.fromCharCode(num);
	},
};
...
class Interpreter {
		...
		if (input) {
			...
			this.input = async () => {
				...
				try {
					...
					const prompt = (promptSync as any)({ sigint: true });
					return prompt(">> ");
				} catch {
					throw new Error(
						"Para usar LEIA() é necessário fornecer função de input ou instalar pacote 'prompt-sync'."
					);
				}
			};
		}
	 ...
	 * Atribuição: x <- expr
	 */
	private async executeAssign(stmt: AssignStmt) {
	 ...
	 */
	private async executeWrite(stmt: WriteStmt) {
	 ...
	 */
	private async executeRead(stmt: ReadStmt) {
		const inputStr: string = await this.input();
		...
		switch (variable.typeName) {
			case "inteiro":
				...
				if (isNaN(parsedValue as number))
					throw new Error(
						`Entrada inválida para inteiro: '${inputStr}'.`
					);
				break;
			case "real":
				...
				if (isNaN(parsedValue as number))
					throw new Error(
						`Entrada inválida para real: '${inputStr}'.`
					);
				break;
			case "logico":
				const low = inputStr.trim().toLowerCase();
				if (low === "verdadeiro" || low === "true") parsedValue = true;
				else if (low === "falso" || low === "false")
					parsedValue = false;
				else
					throw new Error(
						`Entrada inválida para lógico: '${inputStr}'.`
					);
				break;
			case "caractere":
				if (inputStr.length === 0)
					throw new Error("Entrada vazia para caractere.");
			...
			default:
				throw new Error(
					`Tipo desconhecido em LEIA(): ${variable.typeName}.`
				);
		}
	 ...
	 */
	private async executeIf(stmt: IfStmt) {
	 ...
	 */
	private async executeSwitch(stmt: SwitchCaseStmt) {
		const varValue = this.environment.get(stmt.variable).value;
		let matched = false;
		for (const c of stmt.cases) {
			const caseVal = await this.evaluate(c.value);
	 ...
	 */
	private async executeFor(stmt: ForStmt) {
		// Avalia início, fim e passo
		const startValRaw = await this.evaluate(stmt.start);
		const endValRaw = await this.evaluate(stmt.end);
		let stepVal: number = 1;
		if (stmt.step) {
			const stepRaw = await this.evaluate(stmt.step);
			if (typeof stepRaw !== "number")
				throw new Error("Passo em FOR não é numérico.");
			stepVal = stepRaw;
		}
		...
		try {
			...
			if (stepVal > 0) {
				while (
					(this.environment.get(varKey).value as number) <=
					(endValRaw as number)
				) {
					try {
					...
					// Incrementa variável
					const currentVal = this.environment.get(varKey)
						.value as number;
	 ...
	 */
	private async executeWhile(stmt: WhileStmt) {
		while (true) {
			const condVal = await this.evaluate(stmt.condition);
			if (typeof condVal !== "boolean")
				throw new Error("Condição de ENQUANTO não é lógica.");
			if (!condVal) break;
			try {
				for (const s of stmt.body) {
					await this.execute(s);
	 ...
	 */
	private async executeRepeat(stmt: RepeatStmt) {
		while (true) {
			try {
				for (const s of stmt.body) {
					await this.execute(s);
			...
			const condVal = await this.evaluate(stmt.condition);
			if (typeof condVal !== "boolean")
				throw new Error("Condição de ATE em REPITA não é lógica.");
			if (condVal) break;
		}
	}
	 ...
	 */
	private async executeCall(stmt: CallStmt) {
		...
		if (this.functions.has(nameKey)) {
			await this.callFunction(this.functions.get(nameKey)!, stmt.args);
			return;
		}
		// Procedimento do usuário
		if (this.procedures.has(nameKey)) {
			await this.callProcedure(this.procedures.get(nameKey)!, stmt.args);
			return;
		}
	...
	private async callFunction(
		...
		if (argExprs.length !== func.params.length) {
			throw new Error(
				`Função '${func.name}' chamada com número incorreto de argumentos.`
			);
		}
		...
		for (let i = 0; i < argExprs.length; i++) {
			...
			const val = await this.evaluate(argExprs[i]);
	...
	private async callProcedure(
		...
		if (argExprs.length !== proc.params.length) {
			throw new Error(
				`Procedimento '${proc.name}' chamado com número incorreto de argumentos.`
			);
		}
		const previous = this.environment;
		const localEnv = new Environment(this.globals);
	 ...
	 */
	private async evaluate(expr: Expr): Promise<ValueType> {
		...
		if (expr instanceof BinaryExpr) {
			const leftVal = await this.evaluate(expr.left);
			const rightVal = await this.evaluate(expr.right);
		...
		if (expr instanceof FuncCallExpr) {
			...
			const argVals: ValueType[] = [];
			for (const arg of expr.args) {
				argVals.push(await this.evaluate(arg));
			}
			if (builtInFunctions[nameKey]) {
		...
		if (expr instanceof CastExpr) {
			const raw = await this.evaluate(expr.expr);
		...
		switch (operator.type) {
			case TokenType.NOT:
				if (typeof right !== "boolean")
					throw new Error("Operador NOT aplicado a não-lógico.");
				return !right;
			case TokenType.MINUS:
				if (typeof right !== "number")
					throw new Error(
						"Operador unário '-' aplicado a não-numérico."
					);
				return -right;
		}
	...
	private evaluateBinary(
		...
		switch (operator.type) {
			...
			case TokenType.DIV:
				...
				throw new Error(
					"Operador 'DIV' inválido para operandos dados."
				);
			case TokenType.MOD:
			case TokenType.PERCENT:
				...
				throw new Error(
					"Operador 'MOD' inválido para operandos dados."
				);
			...
			case TokenType.XOR:
				if (typeof left === "boolean" && typeof right === "boolean")
					return left !== right;
				throw new Error(
					"Operador 'XOR' inválido para operandos dados."
				);
		}
...
export async function runVisualgProgram(
	...
	try {
		...
		await interpreter.interpret(nodes);
	} catch (e: any) {
		if (output) {
